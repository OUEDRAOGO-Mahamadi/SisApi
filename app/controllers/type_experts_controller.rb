class TypeExpertsController < InheritedResources::Base
  ...
  def create
    @type_expert = TypeExpert.new(type_expert_params)
  ...
  def update
    respond_to do |format|
      if @type_expert.update(type_expert_params)
  ...
  private
    ...
    def set_type_ressource
      @type_expert = TypeExpert.find(params[:id])
      #json_response(@type_expert)
    end

    def type_expert_params
      params.require(:type_expert).permit(:libelle, :sigle, :description)
    end
