class EvenementProfilesController < InheritedResources::Base
  include Response
  ...
  def index
    @ressources = EvenementProfile.all
  ...
  def create
    @ressource = EvenementProfile.new(evenement_profile_params)
  ...
  def update
    ...
    if @ressource.update(evenement_profile_params)
      json_response(@ressource)
    else
  ...
  private
    def set_ressource
      @ressource = EvenementProfile.find(params[:id])
      #json_response(@ressource)
    end

    def evenement_profile_params
      params.require(:evenement_profile).permit(:evenement_id, :profile_id)
    end
