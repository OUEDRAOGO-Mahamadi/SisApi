class RessourceProfilesController < ApplicationController
  include Response
  ...
  def index
    @ressources = RessourceProfile.all
  ...
  def create
    @ressource = RessourceProfile.new(ressource_profile_params)
  ...
  private
    ...
    def set_ressource
      @ressource = RessourceProfile.find(params[:id])
      #json_response(@ressource)
    end

    # Only allow a list of trusted parameters through.
    def ressource_profile_params
      params.require(:ressource_profile).permit(:ressource_id,:profile_id)
    end
end
